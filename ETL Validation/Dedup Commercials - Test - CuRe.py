...
# MAGIC WHERE session_start >= '2023-05-01'
# MAGIC   AND session_start < '2023-07-01'
# MAGIC   AND partition_key >= '2023-05-01'
# MAGIC   AND partition_key < '2023-07-01'
# MAGIC GROUP BY 1
...
# MAGIC FROM prod.detection.viewing_commercials_firehose_dedup c
# MAGIC WHERE c.session_start >= '2024-08-14T00:00:00'::timestamp
# MAGIC         AND c.session_start < '2024-08-15T00:00:00'::timestamp
# MAGIC         AND c.partition_key >= '2024-08-14'
# MAGIC         AND c.partition_key <= '2024-08-15'
# MAGIC GROUP BY 1
...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tivo;
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tivo (
...
# MAGIC     );
# MAGIC
# MAGIC INSERT INTO dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tivo (
...
# MAGIC     WHERE sch.airdate >= '2024-08-16T00:00:00'::timestamp - interval '60' day
# MAGIC ),
...
# MAGIC     WHERE session_start >= '2024-08-16T00:00:00'::timestamp
# MAGIC         AND session_start < '2024-08-17T00:00:00'::timestamp
# MAGIC ),
...
# MAGIC         WHERE c.session_start >= '2024-08-16T00:00:00'::timestamp
# MAGIC             AND c.session_start <= '2024-08-17T00:00:00'::timestamp
# MAGIC     )
...
# MAGIC     END, 
# MAGIC     prev_vizio_station.station_id, 
# MAGIC     prev_vizio_program.program_aggregate_id
# MAGIC     FROM prod.detection.viewing_commercials_firehose_dedup AS c
...
# MAGIC         AND tvis.create_timestamp <= '2024-08-17T00:00:00'::timestamp
# MAGIC         AND tvis.next_create_timestamp >= '2024-08-16T00:00:00'::timestamp
...
# MAGIC         AND tv_settings.create_timestamp <= '2024-08-17T00:00:00'::timestamp
# MAGIC         AND tv_settings.next_create_timestamp >= '2024-08-16T00:00:00'::timestamp
# MAGIC     JOIN 
...
# MAGIC         AND prev_schedule.airdate >= '2024-08-16T00:00:00'::timestamp - interval '60' day
...
# MAGIC         AND next_schedule.airdate >= '2024-08-16T00:00:00'::timestamp - interval '60' day
...
# MAGIC         AND tis.create_timestamp <= ('2024-08-17T00:00:00'::timestamp::double)::timestamp
# MAGIC         AND tis.next_create_timestamp >= ('2024-08-16T00:00:00'::timestamp::double)::timestamp
...
# MAGIC     WHERE
# MAGIC         c.session_start >= '2024-08-16T00:00:00'::timestamp
# MAGIC         AND c.session_start < '2024-08-17T00:00:00'::timestamp
# MAGIC         AND c.partition_key >= '2024-08-16'
# MAGIC         AND c.partition_key <= '2024-08-17'
...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tivo;
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tivo (
# MAGIC     tvid string, hash string, zipcode string, dma string, value string, mt_start integer, ts_start timestamp, ts_end timestamp, prev_episode_id string, prev_title string, prev_ts_start timestamp, prev_ts_end timestamp, prev_channel_callsign string, prev_network_affiliate string, next_episode_id string, next_title string, next_ts_start timestamp, next_ts_end timestamp, next_channel_callsign string, next_network_affiliate string, live string, brand_name string, title string, duration integer, ip_null string, input_category string, input_device string, app_service string, vizio_epg_channel_id string, vizio_epg_program_id string
# MAGIC     );
# MAGIC
# MAGIC INSERT INTO dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tivo (
# MAGIC     tvid, hash, zipcode, dma, value, mt_start, ts_start, ts_end, prev_episode_id, prev_title, prev_ts_start, prev_ts_end, prev_channel_callsign, prev_network_affiliate, next_episode_id, next_title, next_ts_start, next_ts_end, next_channel_callsign, next_network_affiliate, live, brand_name, title, duration, ip_null, input_category, input_device, app_service, vizio_epg_channel_id, vizio_epg_program_id
# MAGIC )
# MAGIC WITH clients_table_to_join AS (
# MAGIC     SELECT *
...
# MAGIC     SELECT DISTINCT fk_tvid, fk_station_id, media_time_start, session_start, session_end, airdate, fk_content_id, is_live
# MAGIC     FROM prod.detection.viewing_content_firehose AS content
# MAGIC     WHERE session_start >= '2024-08-14T00:00:00'::timestamp
# MAGIC         AND session_start < '2024-08-15T00:00:00'::timestamp
# MAGIC ),
# MAGIC content_ids_firehose AS (
# MAGIC     SELECT * FROM detection.content_ids_firehose AS cid
# MAGIC     WHERE content_id IN (
...
# MAGIC     prev_vizio_station.station_id, 
# MAGIC     prev_vizio_program.program_aggregate_id
# MAGIC     FROM dev.detection.viewing_commercials_firehose_dedup AS c
# MAGIC     JOIN prod.detection.zoo AS z 
# MAGIC         ON c.fk_zoo_id = z.zoo_id
# MAGIC         AND z.zoo = 'control-zoo-dtsprod.tvinteractive.tv'
# MAGIC     INNER JOIN
...
# MAGIC         ON c.session_start >= tvis.create_timestamp 
# MAGIC         AND c.session_start < tvis.next_create_timestamp
# MAGIC         AND tvis.create_timestamp <= '2024-08-15T00:00:00'::timestamp
# MAGIC         AND tvis.next_create_timestamp >= '2024-08-14T00:00:00'::timestamp
# MAGIC         AND c.fk_tvid = tvis.fk_tvid   
# MAGIC         AND c.fk_input_source_id = tvis.fk_input_source_id
# MAGIC     JOIN
# MAGIC         prod.detection.tv_settings AS tv_settings
...
# MAGIC         AND c.session_start >= tv_settings.create_timestamp
# MAGIC         AND c.fk_tvid = tv_settings.fk_tvid
# MAGIC         AND tv_settings.create_timestamp <= '2024-08-15T00:00:00'::timestamp
# MAGIC         AND tv_settings.next_create_timestamp >= '2024-08-14T00:00:00'::timestamp
# MAGIC     JOIN 
# MAGIC         prod.detection.settings AS settings
# MAGIC         ON tv_settings.fk_settings_id = settings.settings_id
# MAGIC         AND UPPER(settings.country_name) = 'USA'
...
# MAGIC         ON  c.session_start >=  (tis.create_timestamp::double)::timestamp
# MAGIC         AND c.session_start <  (tis.next_create_timestamp::double)::timestamp
# MAGIC         AND tis.create_timestamp <= ('2024-08-15T00:00:00'::timestamp::double)::timestamp
# MAGIC         AND tis.next_create_timestamp >= ('2024-08-14T00:00:00'::timestamp::double)::timestamp
# MAGIC         AND c.fk_tvid = tis.fk_tvid 
# MAGIC         AND c.fk_input_source_id = tis.fk_input_source_id
# MAGIC     LEFT OUTER JOIN activity_obfuscation appb 
# MAGIC         ON coalesce(tis.app_name) = appb.app_name 
...
# MAGIC         AND c.next_show_id = rep_nyc_nat_next.fk_show_id
# MAGIC     WHERE
# MAGIC         c.session_start >= '2024-08-14T00:00:00'::timestamp
# MAGIC         AND c.session_start < '2024-08-15T00:00:00'::timestamp
# MAGIC         AND c.partition_key >= '2024-08-14'
# MAGIC         AND c.partition_key <= '2024-08-15'
...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tms;
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tms (
# MAGIC     tvid string, hash string, zipcode string, dma string, value string, mt_start integer, ts_start timestamp, ts_end timestamp, prev_episode_id string, prev_title string, prev_ts_start timestamp, prev_ts_end timestamp, prev_channel_callsign string, prev_network_affiliate string, next_episode_id string, next_title string, next_ts_start timestamp, next_ts_end timestamp, next_channel_callsign string, next_network_affiliate string, live string, brand_name string, title string, duration integer, ip_null string, input_category string, input_device string, app_service string, vizio_epg_channel_id string, vizio_epg_program_id string
# MAGIC     );
# MAGIC
# MAGIC INSERT INTO dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tms (
# MAGIC     tvid, hash, zipcode, dma, value, mt_start, ts_start, ts_end, prev_episode_id, prev_title, prev_ts_start, prev_ts_end, prev_channel_callsign, prev_network_affiliate, next_episode_id, next_title, next_ts_start, next_ts_end, next_channel_callsign, next_network_affiliate, live, brand_name, title, duration, ip_null, input_category, input_device, app_service, vizio_epg_channel_id, vizio_epg_program_id
# MAGIC )
# MAGIC WITH clients_table_to_join AS (
# MAGIC     SELECT *
...
# MAGIC     SELECT DISTINCT *
# MAGIC     FROM prod.detection.epg_schedule_latest sch
# MAGIC     WHERE sch.airdate >= '2024-08-14T00:00:00'::timestamp - interval '60' day
# MAGIC ),
# MAGIC epg_program_aggregate AS (
# MAGIC     SELECT DISTINCT *
# MAGIC     FROM prod.detection.vizio_epg_program_aggregate 
...
# MAGIC     prev_vizio_station.station_id, 
# MAGIC     prev_vizio_program.program_aggregate_id
# MAGIC     FROM detection.viewing_commercials_firehose_dedup AS c
# MAGIC     JOIN prod.detection.zoo AS z 
# MAGIC         ON c.fk_zoo_id = z.zoo_id
# MAGIC         AND z.zoo = 'control-zoo-dtsprod.tvinteractive.tv'
# MAGIC     INNER JOIN
...
# MAGIC         AND timestampadd(SECOND,  prev_content.media_time_start,  prev_content.airdate) >= prev_schedule.airdate  
# MAGIC         AND timestampadd(SECOND,  prev_content.media_time_start,  prev_content.airdate) < prev_schedule.airdate_end
# MAGIC         AND prev_schedule.airdate >= '2024-08-14T00:00:00'::timestamp - interval '60' day
# MAGIC     LEFT OUTER JOIN prod.detection.epg_show AS prev_program
# MAGIC         ON prev_schedule.fk_show_id = prev_program.show_id
# MAGIC     LEFT OUTER JOIN prod.detection.epg_show AS prev_program_alt
# MAGIC         ON c.prev_show_id = prev_program_alt.show_id
...
# MAGIC         AND timestampadd(SECOND,  next_content.media_time_start,  next_content.airdate) >= next_schedule.airdate  
# MAGIC         AND timestampadd(SECOND,  next_content.media_time_start,  next_content.airdate) < next_schedule.airdate_end
# MAGIC         AND next_schedule.airdate >= '2024-08-14T00:00:00'::timestamp - interval '60' day
# MAGIC     LEFT OUTER JOIN prod.detection.epg_show AS next_program
# MAGIC         ON next_schedule.fk_show_id = next_program.show_id
# MAGIC     LEFT OUTER JOIN prod.detection.epg_show AS next_program_alt
# MAGIC         ON c.next_show_id = next_program_alt.show_id
...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tms;
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tms (
# MAGIC     tvid string, hash string, zipcode string, dma string, value string, mt_start integer, ts_start timestamp, ts_end timestamp, prev_episode_id string, prev_title string, prev_ts_start timestamp, prev_ts_end timestamp, prev_channel_callsign string, prev_network_affiliate string, next_episode_id string, next_title string, next_ts_start timestamp, next_ts_end timestamp, next_channel_callsign string, next_network_affiliate string, live string, brand_name string, title string, duration integer, ip_null string, input_category string, input_device string, app_service string, vizio_epg_channel_id string, vizio_epg_program_id string
# MAGIC     );
# MAGIC
# MAGIC INSERT INTO dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tms (
# MAGIC     tvid, hash, zipcode, dma, value, mt_start, ts_start, ts_end, prev_episode_id, prev_title, prev_ts_start, prev_ts_end, prev_channel_callsign, prev_network_affiliate, next_episode_id, next_title, next_ts_start, next_ts_end, next_channel_callsign, next_network_affiliate, live, brand_name, title, duration, ip_null, input_category, input_device, app_service, vizio_epg_channel_id, vizio_epg_program_id
# MAGIC )
# MAGIC WITH clients_table_to_join AS (
# MAGIC     SELECT *
...
# MAGIC FROM dev.mohit_gangwani.deduped_commercial_feed_cognet_existing_table_tivo AS exs_report
# MAGIC JOIN dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tivo AS new_report
...
# MAGIC  FROM dev.mohit_gangwani.deduped_commercial_feed_cognet_new_table_tivo AS new_report
# MAGIC  GROUP BY 2
# MAGIC )
# MAGIC ORDER BY 2, 1
...
# MAGIC FROM (
# MAGIC select fk_tvid, session_start, session_end, media_time_start, count(*) AS cnt
# MAGIC from dev.detection.viewing_content_firehose 
# MAGIC where session_start >= '2024-08-25 18:00:00'
# MAGIC AND tms_station_id IS NOT NULL
# MAGIC group by fk_tvid, session_start, session_end, media_time_start)
# MAGIC GROUP BY 1
# MAGIC
