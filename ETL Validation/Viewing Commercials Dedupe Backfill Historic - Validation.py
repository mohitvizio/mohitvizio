...
# MAGIC %sql
# MAGIC
# MAGIC SELECT * FROM dev.historic.viewing_commercials_firehose_dedup
...
# MAGIC FROM dev.historic.viewing_commercials_firehose_dedup
# MAGIC WHERE fk_zoo_id = 17
# MAGIC GROUP BY 1
...
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.prev_tms_show_id IS NULL     AND vc.next_tms_show_id IS NULL THEN 'Prev and Next Null'
# MAGIC          WHEN vc.prev_tms_show_id IS NULL     AND vc.next_tms_show_id IS NOT NULL THEN 'Prev Null, Next not Null'
# MAGIC          WHEN vc.prev_tms_show_id IS NOT NULL AND vc.next_tms_show_id IS NULL THEN 'Prev not Null, Next Null'
# MAGIC          WHEN vc.prev_tms_show_id IS NOT NULL AND vc.next_tms_show_id IS NOT NULL THEN 'Prev and Next not Null'
# MAGIC   END AS show_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.prev_show_id IS NULL     AND vc.prev_tms_show_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.prev_show_id IS NULL     AND vc.prev_tms_show_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.prev_show_id IS NOT NULL AND vc.prev_tms_show_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.prev_show_id IS NOT NULL AND vc.prev_tms_show_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS show_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.next_show_id IS NULL     AND vc.next_tms_show_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.next_show_id IS NULL     AND vc.next_tms_show_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.next_show_id IS NOT NULL AND vc.next_tms_show_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.next_show_id IS NOT NULL AND vc.next_tms_show_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS show_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   WHERE vc.fk_zoo_id=17vc.fk_zoo_id=17
# MAGIC     -- AND vc.partition_key >= '2024-08-02'
# MAGIC     -- AND vc.partition_key <= '2024-08-07'
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.prev_station_id IS NULL     AND vc.prev_tms_station_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.prev_station_id IS NULL     AND vc.prev_tms_station_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.prev_station_id IS NOT NULL AND vc.prev_tms_station_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.prev_station_id IS NOT NULL AND vc.prev_tms_station_id IS NOT NULL THEN 'TiVo and TMS not Null'
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.next_station_id IS NULL     AND vc.next_tms_station_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.next_station_id IS NULL     AND vc.next_tms_station_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.next_station_id IS NOT NULL AND vc.next_tms_station_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.next_station_id IS NOT NULL AND vc.next_tms_station_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS station_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN st.station_id IS NOT NULL THEN 1 ELSE 0 END AS station_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
...
# MAGIC     ON st.station_id = vc.next_station_id
# MAGIC    AND st.vendor_name = 'TIVO'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.next_station_id IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN detection.epg_station st
# MAGIC     ON st.station_id = vc.prev_station_id
# MAGIC    AND st.vendor_name = 'TIVO'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.prev_station_id IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN detection.epg_station st
# MAGIC     ON st.station_id = vc.prev_tms_station_id
# MAGIC    AND st.vendor_name = 'TMS'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.prev_tms_station_id IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN detection.epg_station st
# MAGIC     ON st.station_id = vc.next_tms_station_id
# MAGIC    AND st.vendor_name = 'TMS'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.next_tms_station_id IS NOT NULL
# MAGIC GROUP BY 1, 2

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT vc.prev_show_id
# MAGIC   -- SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   -- , CASE WHEN sh.show_id IS NOT NULL THEN 1 ELSE 0 END AS show_id_check
# MAGIC   -- , COUNT(*)*1.0 AS sessions_count
# MAGIC   -- , COUNT(Distinct vc.fk_show_id) AS total_shows
# MAGIC   -- , SUM(vc.session_duration)/3600.0 AS total_duration
# MAGIC   -- , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN prod.detection.epg_show sh
# MAGIC     ON sh.show_id = CAST(vc.prev_show_id AS BIGINT)
# MAGIC    AND sh.vendor_name = 'TIVO'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.prev_show_id IS NOT NULL
# MAGIC     AND sh.show_id IS NULL
# MAGIC     -- AND vc.session_start >= '2023-01-01T00:00:00'
# MAGIC     -- AND vc.session_start < '2023-01-02T00:00:00'
# MAGIC     -- AND vc.partition_key = '2023-01-01'
# MAGIC GROUP BY 1-- , 2
# MAGIC LIMIT 100
...
# MAGIC WHERE vendor_name = 'TIVO'
# MAGIC AND database_key = '11868840668'
...
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN detection.epg_show sh
# MAGIC     ON sh.show_id = vc.prev_tms_show_id
# MAGIC    AND sh.vendor_name = 'TMS'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.prev_tms_show_id IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC   FROM dev.historic.viewing_commercials_firehose_dedup vc
# MAGIC   LEFT JOIN detection.epg_show sh
# MAGIC     ON sh.show_id = vc.next_tms_show_id
# MAGIC    AND sh.vendor_name = 'TMS'
# MAGIC   WHERE vc.fk_zoo_id=17
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.next_tms_show_id IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC   , CASE WHEN sh.show_id IS NOT NULL THEN 1 ELSE 0 END AS show_id_check
# MAGIC   , COUNT(*)
# MAGIC   , COUNT(DISTINCT vc.fk_show_id) AS missing_show_ids
# MAGIC   FROM dev.historic.viewing_content_firehose vc
# MAGIC   LEFT JOIN detection.epg_show sh
...
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND vc.session_start >= '2023-01-01'
# MAGIC     AND vc.session_start < '2023-01-02'
# MAGIC     AND vc.partition_key = '2023-01-01'
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT * FROM detection.epg_station
# MAGIC WHERe station_id IN (203871, 2512)

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC SELECT * FROM detection.inscape_station_map
# MAGIC WHERe inscape_station_id IN (203871, 2512)
