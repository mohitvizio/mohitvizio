...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.r388_content_with_null_cognet_existing_table;
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.r388_content_with_null_cognet_existing_table (
...
# MAGIC     );
# MAGIC INSERT INTO dev.mohit_gangwani.r388_content_with_null_cognet_existing_table (
# MAGIC     tvid, 
# MAGIC     hash, 
# MAGIC     zipcode, 
# MAGIC     dma, 
...
# MAGIC     JOIN prod.detection.tv AS tv ON c.session_start >= '2024-08-14T07:00:00'::timestamp
# MAGIC         AND c.session_start < '2024-08-15T07:00:00'::timestamp
...
# MAGIC         AND tv_settings.create_timestamp <= '2024-08-15T07:00:00'::timestamp
# MAGIC         AND tv_settings.next_create_timestamp >= '2024-08-14T07:00:00'::timestamp
...
# MAGIC         AND schedule.airdate >= '2024-08-14T07:00:00'::timestamp - interval '60' day
...
# MAGIC         AND tvis.create_timestamp <= '2024-08-15T07:00:00'::timestamp
# MAGIC         AND tvis.next_create_timestamp >= '2024-08-14T07:00:00'::timestamp
...
# MAGIC         AND tis.create_timestamp <= ('2024-08-15T07:00:00'::timestamp::double)::timestamp 
# MAGIC         AND tis.next_create_timestamp >= ('2024-08-14T07:00:00'::timestamp::double)::timestamp 
...
# MAGIC         AND ip.create_timestamp <= '2024-08-15T07:00:00'::timestamp
# MAGIC         AND ip.next_create_timestamp >= '2024-08-14T07:00:00'::timestamp
...
# MAGIC     WHERE
# MAGIC         c.session_start >= '2024-08-14T07:00:00'::timestamp
# MAGIC     AND c.session_start < '2024-08-15T07:00:00'::timestamp
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.r388_content_with_null_cognet_new_table;
# MAGIC CREATE TABLE IF NOT EXISTS dev.mohit_gangwani.r388_content_with_null_cognet_new_table (
# MAGIC     tvid string, hash string, zipcode string, dma string, episode_id string, show_title string, air_date string, channel_callsign string, mt_start integer, ts_start timestamp, ts_end timestamp, channel_affiliate string, live string, ip string, input_category string, input_device string, app_service string
# MAGIC     );
# MAGIC
# MAGIC INSERT INTO dev.mohit_gangwani.r388_content_with_null_cognet_new_table (
# MAGIC     tvid, 
# MAGIC     hash, 
# MAGIC     zipcode, 
# MAGIC     dma, 
...
# MAGIC  , DATE(tms_report.ts_start) AS report_date
# MAGIC  , COUNT(*) AS session_count
# MAGIC  , COUNT(DISTINCT tms_report.tvid) AS total_tvs
# MAGIC  , SUM(TIMESTAMPDIFF(SECOND, tms_report.ts_start, tms_report.ts_end))/3600.0 AS ttl_hrs
# MAGIC  FROM dev.mohit_gangwani.r388_content_with_null_cognet_existing_table AS tms_report
# MAGIC  JOIN dev.mohit_gangwani.r388_content_with_null_cognet_new_table AS tivo_report
# MAGIC   ON tms_report.tvid = tivo_report.tvid
# MAGIC  AND tms_report.ts_start = tivo_report.ts_start
# MAGIC  AND tms_report.ts_end = tivo_report.ts_end
# MAGIC  AND tms_report.ip <=> tivo_report.ip
# MAGIC  GROUP BY 2
# MAGIC )
# MAGIC UNION
...
# MAGIC  , DATE(ts_start) AS report_date
# MAGIC  , COUNT(*) AS total_matching_rows
# MAGIC  , COUNT(DISTINCT tivo_report.tvid) AS total_tvs
# MAGIC  , SUM(TIMESTAMPDIFF(SECOND, tivo_report.ts_start, tivo_report.ts_end))/3600.0 AS ttl_hrs
# MAGIC  FROM dev.mohit_gangwani.r388_content_with_null_cognet_new_table AS tivo_report
# MAGIC  GROUP BY 2
# MAGIC )
# MAGIC ORDER BY 2, 1
...
# MAGIC , COUNT(DISTINCT exs_report.tvid) AS total_tvs
# MAGIC FROM dev.mohit_gangwani.r388_content_with_null_cognet_existing_table AS exs_report
# MAGIC JOIN dev.mohit_gangwani.r388_content_with_null_cognet_new_table AS new_report
...
# MAGIC --- should return 0.
# MAGIC select count(*)
# MAGIC from dev.mohit_gangwani.r388_content_with_null_cognet_new_table
