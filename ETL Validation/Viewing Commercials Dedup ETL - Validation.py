...
# MAGIC WHERE session_start >= '2024-10-07T00:00:00'
...
# MAGIC   FROM prod.detection.viewing_commercials_firehose_dedup vc
# MAGIC   WHERE vc.session_start>='2024-10-19T03:00:00'
# MAGIC     AND vc.session_start < '2024-10-19T14:00:00'
# MAGIC     AND vc.fk_zoo_id=17
# MAGIC     -- AND vc.partition_key = '2024-10-13'
# MAGIC     -- AND vc.partition_key <= '2024-10-13'
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND vc.fk_commercial_source_id = 1
...
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM dev.detection.viewing_commercials_firehose_dedup vc
# MAGIC   WHERE vc.session_start>='2024-10-19T03:00:00'
# MAGIC     AND vc.session_start < '2024-10-19T14:00:00'
# MAGIC     AND vc.fk_zoo_id=17
# MAGIC     -- AND vc.partition_key = '2024-10-13'
...
# MAGIC , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC FROM prod.detection.viewing_commercials_firehose_dedup vc
# MAGIC   WHERE vc.session_start>='2024-10-17T03:00:00'
# MAGIC     AND vc.session_start < '2024-10-17T14:00:00'
...
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM dev.detection.viewing_commercials_firehose_dedup vc
# MAGIC WHERE vc.session_start >= '2024-10-12T00:00:00'
# MAGIC   AND vc.partition_key = '2024-10-12'
# MAGIC   -- WHERE vc.session_start>='2024-08-02T00:00:00'
# MAGIC   --   AND vc.session_start < '2024-08-07T00:00:00'
# MAGIC     AND vc.fk_zoo_id=17
# MAGIC     -- AND vc.partition_key >= '2024-08-02'
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('HOUR', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.next_station_id IS NULL     AND vc.tms_next_station_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.next_station_id IS NULL     AND vc.tms_next_station_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.next_station_id IS NOT NULL AND vc.tms_next_station_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.next_station_id IS NOT NULL AND vc.tms_next_station_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS station_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
