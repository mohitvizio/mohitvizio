...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('MONTH', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.prev_tuner_program_id IS NULL     AND vc.tms_prev_tuner_program_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.prev_tuner_program_id IS NULL     AND vc.tms_prev_tuner_program_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.prev_tuner_program_id IS NOT NULL AND vc.tms_prev_tuner_program_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.prev_tuner_program_id IS NOT NULL AND vc.tms_prev_tuner_program_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS show_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
...
# MAGIC   FROM prod.detection.viewing_commercials_firehose vc
# MAGIC   WHERE 
# MAGIC   -- vc.session_start >= '2023-10-01T00:00:00'
# MAGIC   -- AND vc.session_start < '2024-02-01T00:00:00.000'
# MAGIC   --   AND 
# MAGIC     vc.partition_key >= '2023-05-01'
# MAGIC     AND vc.partition_key <= '2024-01-01'
# MAGIC   -- WHERE vc.session_start>='2024-08-02T00:00:00'
# MAGIC   --   AND vc.session_start < '2024-08-07T00:00:00'
# MAGIC     AND vc.fk_zoo_id=17
# MAGIC     -- AND vc.partition_key >= '2024-08-02'
...
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND COALESCE(vc.prev_tuner_program_id, vc.tms_prev_tuner_program_id) IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('MONTH', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.next_tuner_program_id IS NULL     AND vc.tms_next_tuner_program_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.next_tuner_program_id IS NULL     AND vc.tms_next_tuner_program_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.next_tuner_program_id IS NOT NULL AND vc.tms_next_tuner_program_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.next_tuner_program_id IS NOT NULL AND vc.tms_next_tuner_program_id IS NOT NULL THEN 'TiVo and TMS not Null'
# MAGIC   END AS show_id_check
# MAGIC   , COUNT(*)*1.0 AS sessions_count
# MAGIC   , COUNT(Distinct vc.fk_tvid) AS total_tvs
# MAGIC   , SUM(vc.session_duration)/3600.0 AS total_duration
...
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND COALESCE(vc.next_tuner_program_id, vc.tms_next_tuner_program_id) IS NOT NULL
# MAGIC GROUP BY 1, 2

# COMMAND ----------

# DBTITLE 1,TMS + TiVo Previous Station ID Check
# MAGIC %sql
# MAGIC   SELECT DATE_TRUNC('DAY', vc.session_start) AS session_hour
# MAGIC   , CASE WHEN vc.prev_tuner_channel_id IS NULL     AND vc.tms_prev_tuner_channel_id IS NULL THEN 'TiVo and TMS Null'
# MAGIC          WHEN vc.prev_tuner_channel_id IS NULL     AND vc.tms_prev_tuner_channel_id IS NOT NULL THEN 'TiVo Null, TMS not Null'
# MAGIC          WHEN vc.prev_tuner_channel_id IS NOT NULL AND vc.tms_prev_tuner_channel_id IS NULL THEN 'TiVo not Null, TMS Null'
# MAGIC          WHEN vc.prev_tuner_channel_id IS NOT NULL AND vc.tms_prev_tuner_channel_id IS NOT NULL THEN 'TiVo and TMS not Null'
...
# MAGIC   , COUNT(DISTINCT vc.fk_commercial_id) AS dist_comm_count
# MAGIC   FROM prod.detection.viewing_commercials_firehose vc
# MAGIC   WHERE vc.session_start >= '2023-05-01T00:00:00'
# MAGIC   AND vc.session_start < '2024-01-01T00:00:00.000'
# MAGIC   AND vc.partition_key >= '2023-05-01'
# MAGIC     AND vc.partition_key <= '2024-01-01'
# MAGIC   -- WHERE vc.session_start>='2024-08-02T00:00:00'
# MAGIC   --   AND vc.session_start < '2024-08-07T00:00:00'
# MAGIC     AND vc.fk_zoo_id=17
...
# MAGIC     AND vc.session_duration > 0
# MAGIC     AND MOD(vc.fk_tvid, 100) = 0
# MAGIC     AND COALESCE(vc.prev_tuner_channel_id, vc.tms_prev_tuner_channel_id) IS NOT NULL
# MAGIC GROUP BY 1, 2
...
# MAGIC     JOIN detection.tv ON tv.tvid = vc.fk_tvid          
# MAGIC     WHERE vc.fk_tvid = 164555016
# MAGIC       AND vc.session_start >= '2024-10-01' 
# MAGIC       AND vc.session_start < '2024-11-01'
# MAGIC       AND vc.partition_key >= '2024-10-01' 
# MAGIC       AND vc.partition_key < '2024-11-01'
# MAGIC       AND tv.model_name NOT IN (
# MAGIC         'test', '', 'Convert_Board', 'E60_DV_Panel-less', 'E60_FY17_DV_Panel-less', 'M70_FY17_POH_Panel-less', 
# MAGIC         'Test_28', 'M70_Panel-less', 'E40-E0', 'D43-E2', 'E80u-D3', 'D65-E0', 'D55-E0', 'E55-F0', 'V707-G3', 
# MAGIC         'D50x-G1', 'D24h-F1'
# MAGIC       )
# MAGIC       AND NOT (tv.model_name IN ('V556-G1', 'D50-E1') AND tv.chipset = 'SIGMA_SX7C')
# MAGIC       AND NOT (tv.model_name IN ('E75-F2', 'M65-F0', 'M55-F0', 'E65-F1', 'E65-F0') AND tv.chipset = '5597')
# MAGIC       AND NOT (tv.model_name IN ('D50-F1') AND tv.chipset = '5581p')
# MAGIC       AND tv.model_name NOT ILIKE 'test%'
# MAGIC       AND tv.chipset IN ('SIGMA_SX7C', '5597', '5581p', '5691', '5695', '5695S', '5583', 'NVT72690')
# MAGIC       AND vc.fk_input_source_id NOT IN (47, 32, 34425, 44, 48)

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE dev.mohit_gangwani.viewing_commercials_firehose_for_test AS
# MAGIC SELECT *
# MAGIC FROM prod.detection.viewing_commercials_firehose vc
# MAGIC WHERE vc.session_start >= '2023-11-01 00:00:00'          -- Start Date
# MAGIC   AND vc.session_start < '2023-11-02 00:00:00'           -- End Date
# MAGIC   AND vc.partition_key >= '2023-11-01'
# MAGIC   AND vc.partition_key <= '2023-11-02'
# MAGIC   AND MOD(vc.fk_tvid, 100) = 0;

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT target.tms_prev_tuner_program_id IS NULL, COUNT(*)
# MAGIC FROM dev.mohit_gangwani.viewing_commercials_firehose_for_test AS target
# MAGIC GROUP BY 1

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT DATE_TRUNC('MONTH', session_start), vc.tms_prev_tuner_program_id IS NULL, COUNT(*)
# MAGIC FROM prod.detection.viewing_commercials_firehose AS vc
# MAGIC WHERE vc.session_start >= '2023-05-01 00:00:00'          -- Start Date
# MAGIC   AND vc.session_start < '2023-06-01 00:00:00'           -- End Date
# MAGIC   AND vc.partition_key >= '2023-05-01'
# MAGIC   AND vc.partition_key <= '2023-06-01'
# MAGIC   AND MOD(vc.fk_tvid, 100) = 0
# MAGIC GROUP BY 1, 2
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS dev.mohit_gangwani.temp_viewing_content_for_update;
# MAGIC CREATE TABLE dev.mohit_gangwani.temp_viewing_content_for_update AS
# MAGIC SELECT vc.fk_tvid, vc.session_start, vc.session_end, vc.tuner_program_id, vc.tuner_channel_id, vc.tms_tuner_program_id, vc.tms_tuner_channel_id
# MAGIC FROM prod.detection.viewing_content_firehose vc           -- New table
# MAGIC WHERE vc.session_start >= '2023-11-01 00:00:00'          -- Start Date
# MAGIC   AND vc.session_start < '2023-11-02 00:00:00'           -- End Date
# MAGIC   AND vc.partition_key >= '2023-11-01'
# MAGIC   AND vc.partition_key <= '2023-11-02'
# MAGIC   AND MOD(vc.fk_tvid, 100) = 0
# MAGIC   AND COALESCE(vc.tuner_program_id, vc.tuner_channel_id) IS NOT NULL
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT tivo_tuner_prev.tms_tuner_program_id IS NULL, tivo_tuner_prev.tuner_program_id IS NULL, COUNT(*)
# MAGIC FROM dev.mohit_gangwani.viewing_commercials_firehose_for_test AS target
# MAGIC JOIN dev.mohit_gangwani.temp_viewing_content_for_update AS tivo_tuner_prev
# MAGIC   ON target.fk_tvid = tivo_tuner_prev.fk_tvid
# MAGIC  AND target.prev_session_start = tivo_tuner_prev.session_start
# MAGIC  AND target.prev_session_end = tivo_tuner_prev.session_end
# MAGIC GROUP BY 1, 2
...
# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS temp_viewing_content_for_update;
# MAGIC
# MAGIC
# MAGIC
# MAGIC -------------------------------------------------------------------------------
# MAGIC
# MAGIC MERGE INTO prod.detection.viewing_commercials_firehose AS target
# MAGIC USING temp_viewing_content_for_update AS tivo_tuner_prev
# MAGIC   ON target.fk_tvid = tivo_tuner_prev.fk_tvid
# MAGIC  AND target.prev_session_start = tivo_tuner_prev.session_start
# MAGIC  AND target.prev_session_end = tivo_tuner_prev.session_end
# MAGIC WHEN MATCHED THEN UPDATE SET target.prev_tuner_channel_id = tivo_tuner.tuner_channel_id
# MAGIC , target.prev_tuner_program_id = tivo_tuner.tuner_program_id
# MAGIC WHERE target.session_start >= '2023-11-01 00:00:00'          -- Start Date
# MAGIC   AND target.session_start < '2023-12-01 00:00:00'           -- End Date
# MAGIC   AND target.partition_key >= '2023-11-01'
# MAGIC   AND target.partition_key <= '2023-12-01'
# MAGIC   AND COALESCE(target.tms_prev_tuner_program_id, target.tms_prev_tuner_channel_id) IS NOT NULL
# MAGIC ;
# MAGIC
# MAGIC MERGE INTO prod.detection.viewing_commercials_firehose AS target
# MAGIC USING temp_viewing_content_for_update AS tivo_tuner_next
# MAGIC   ON target.fk_tvid = tivo_tuner_next.fk_tvid
# MAGIC  AND target.next_session_start = tivo_tuner_next.session_start
# MAGIC  AND target.next_session_end = tivo_tuner_next.session_end
# MAGIC WHEN MATCHED THEN UPDATE SET target.next_tuner_channel_id = tivo_tuner.tuner_channel_id
# MAGIC , target.next_tuner_program_id = tivo_tuner.tuner_program_id
# MAGIC WHERE target.session_start >= '2023-11-01 00:00:00'          -- Start Date
# MAGIC   AND target.session_start < '2023-12-01 00:00:00'           -- End Date
# MAGIC   AND target.partition_key >= '2023-11-01'
# MAGIC   AND target.partition_key <= '2023-12-01'
# MAGIC   AND COALESCE(target.tms_next_tuner_program_id, target.tms_next_tuner_channel_id) IS NOT NULL
# MAGIC ;
# MAGIC
# MAGIC DROP TABLE temp_viewing_content_for_update;
