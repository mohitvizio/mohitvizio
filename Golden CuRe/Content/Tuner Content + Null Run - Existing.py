...
table_name = f'existing_tuner_contentnull_{client_name}_{vendor.lower()}_'
...
tuner_station_id = 'c.tuner_channel_id' if vendor == 'TIVO' else 'c.tms_tuner_channel_id'

other_station_id = 'c.tms_station_id' if vendor == 'TIVO' else 'c.fk_station_id'
other_tuner_station_id = 'c.tms_tuner_channel_id' if vendor == 'TIVO' else 'c.tuner_channel_id'

show_id = 'c.fk_show_id' if vendor == 'TIVO' else 'c.tms_show_id'
tuner_show_id = 'c.tuner_program_id' if vendor == 'TIVO' else 'c.tms_tuner_program_id'

other_show_id = 'c.tms_show_id' if vendor == 'TIVO' else 'c.fk_show_id'
other_tuner_show_id = 'c.tms_tuner_program_id' if vendor == 'TIVO' else 'c.tuner_program_id'

airdate = 'c.airdate' if vendor == 'TIVO' else 'c.tms_airdate'
other_airdate = 'c.tms_airdate' if vendor == 'TIVO' else 'c.airdate'
...
SELECT /*+ BROADCAST(cid) */  DISTINCT
    ...
    WHEN c.vizio_epg_station IS NOT NULL THEN
        CASE WHEN chanb.channel_name IS NOT NULL OR c.vizio_epg_station in ('98989898989898', '9898989898', '-1') OR 'TMS' != '{vendor}' THEN NULL
    ...
    CASE
        ...
        WHEN c.vizio_epg_station IS NOT NULL THEN COALESCE({airdate}, {other_airdate})
        WHEN (cl.client_id is not null) THEN NULL

        WHEN nielsen_blacklist.station_id IS NOT NULL AND (NVL(rep_local.station_id, rep_nyc_nat.station_id) IS NULL OR ingest_time IS NOT NULL) THEN NULL
        WHEN station_blacklist.station_id IS NOT NULL THEN NULL 
        WHEN '{client_name}' != 'nielsen' THEN COALESCE({airdate}, {other_airdate})
        ELSE c.tms_airdate
    END, 
    CASE
        WHEN c.vizio_epg_station IS NULL AND acrb.app_name IS NOT NULL THEN NULL
        ...
        WHEN '{client_name}' = 'nielsen' THEN map.inscape_call_sign
        ELSE NULL 
    END,
    CASE
        WHEN c.vizio_epg_station IS NULL AND acrb.app_name IS NOT NULL THEN NULL
        ...
        WHEN nielsen_blacklist.station_id IS NOT NULL AND (NVL(rep_local.station_id, rep_nyc_nat.station_id) IS NULL OR ingest_time IS NOT NULL) THEN NULL
        WHEN station_blacklist.station_id IS NOT NULL THEN NULL 
        WHEN '{client_name}' != 'nielsen' AND COALESCE(c.tms_airdate, c.airdate) IS NULL THEN NULL 
        -- WHEN '{client_name}' = 'nielsen' AND c.tms_airdate IS NULL THEN NULL
        WHEN COALESCE({tuner_station_id}, {other_tuner_station_id}) IS NOT NULL THEN LEAST((unix_timestamp(c.session_start)-unix_timestamp(COALESCE({airdate}, {other_airdate}))), c.runtime) 
    ...
    ELSE
        CASE 
            ...
            -- WHEN '{client_name}' = 'nielsen' AND c.tms_airdate IS NULL THEN NULL
            WHEN '{client_name}' != 'nielsen' AND COALESCE(c.tms_airdate, c.airdate) IS NULL THEN NULL
            WHEN COALESCE({station_id}, {tuner_station_id}) IS NOT NULL THEN
            ...
            WHEN COALESCE({station_id}, {tuner_station_id}) IS NULL AND COALESCE({other_station_id}, {other_tuner_station_id}) IS NOT NULL THEN
    ...
    CASE
        ...
        WHEN nielsen_blacklist.station_id IS NOT NULL AND (NVL(rep_local.station_id, rep_nyc_nat.station_id) IS NULL OR ingest_time IS NOT NULL) THEN NULL
		WHEN station_blacklist.station_id IS NOT NULL THEN NULL
        WHEN COALESCE(c.tuner_channel_id, NULLIF(c.tms_tuner_channel_id,98989898)) IS NOT NULL THEN 't'
    ...
    ip.ip_address, 
    CASE
        WHEN COALESCE({tuner_station_id}, {other_tuner_station_id}) IS NOT NULL AND UPPER(tvis.category) in ('APPS', 'OTHER', 'OTT')
        THEN 'HD TV'
    ...
    END, 
    CASE
        WHEN COALESCE({tuner_station_id}, {other_tuner_station_id}) IS NOT NULL
        THEN 'OTA'
    ...
    CASE 
        ...
        WHEN c.is_live = true AND LOWER(tvis.input_device) in ('xbox','amazon_fire','apple_tv','chromecast','nintendo switch','playstation 4','playstation 5','roku') AND COALESCE({tuner_station_id}, {other_tuner_station_id}) is NULL 
            THEN 'vMVPD' 
...
FROM
    ...
    LEFT OUTER JOIN inscape_station_map_dedupe AS map
        ON map.mapped_vendor_station_id = COALESCE({tuner_station_id}, {station_id})
        AND map.mapped_vendor = '{vendor}'
    LEFT OUTER JOIN prod.detection.epg_station AS station
        ON station.station_id = COALESCE({tuner_station_id}, {station_id})
        AND station.vendor_name = '{vendor}'
    LEFT OUTER JOIN prod.detection.epg_show AS show
        ON show.show_id = COALESCE({tuner_show_id}, {show_id})
        AND show.vendor_name = '{vendor}'
    LEFT OUTER JOIN prod.detection.epg_show AS backup_show
        ON backup_show.show_id = COALESCE({other_tuner_show_id}, {other_show_id})
        AND COALESCE({tuner_show_id}, {show_id}) IS NULL
    ...
    LEFT OUTER JOIN prod.detection.epg_station AS backup_station
        ON backup_station.station_id = COALESCE({other_tuner_station_id}, {other_show_id})
        AND COALESCE({tuner_station_id}, {station_id}) IS NULL
        AND backup_station.vendor_name = '{other_vendor}'
        AND '{client_name}' != 'nielsen'
    LEFT OUTER JOIN inscape_station_map_dedupe AS backup_map
        ON backup_map.mapped_vendor_station_id = COALESCE({other_tuner_station_id}, {other_station_id})
        AND COALESCE({tuner_station_id}, {station_id}) IS NULL
        AND backup_map.mapped_vendor = '{other_vendor}'
        AND '{client_name}' != 'nielsen'
    LEFT OUTER JOIN station_distribution_blacklist AS station_blacklist
        ON COALESCE({tuner_station_id}, {station_id}) = station_blacklist.station_id
    ...
    LEFT OUTER JOIN prod.detection.station_metadata_obfuscation AS station_obfs
        ON COALESCE({tuner_station_id}, {station_id}) = station_obfs.vendor_station_id
