...
schema = 'dev.vlad'
new_report = 'r46052_content_with_null_univision_2025_07_23_07_qa'
existing_report = 'r1149_content_with_null_univision_2025_07_23_07_production'
vendor_name = 'TMS'
tuner_included = True
...
field_to_check = f'show_genre{suff}'
spark.sql(f"""
WITH mm AS (
SELECT exs_report.tvid
, exs_report.ts_start
...
--  AND exs_report.app_service IS NULL
--  AND new_report.app_service IS NULL
 )
 (SELECT exs_report.tvid
...
# MAGIC WHERE fk_tvid = 153518080
# MAGIC AND session_start = '2025-07-28T17:00:00.000+00:00'

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM prod.detection.viewing_content_firehose
# MAGIC WHERE fk_tvid = 102410541	
# MAGIC AND session_start = '2025-07-25 20:16:44'

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM detection.epg_show
# MAGIC WHERE show_id = 8825465
...
# MAGIC  AND tuner.ip_address <=> mt.ip_address
...
spark.sql(f"""
SELECT exs_report.show_genre{suff} AS prod_title
, new_report.show_genre{suff}      AS golden_report_title
...
, COUNT(*)
 ...
 AND exs_report.ip <=> new_report.ip
 AND exs_report.mt_start <=> new_report.mt_start
 AND NOT(exs_report.show_genre <=> new_report.show_genre)
...
existing_report = 'r1149_content_with_null_univision_2025_07_23_07_production'
spark.sql(f"""
          SELECT gt.tuner_app_service, gt.app_service, gt.vod_station, gt.acrb_clients, gt.appb_clients, gt.client_id_not_null, COUNT(*)
-- SELECT gt.*
-- SELECT exs.session_hour, COUNT(*)
FROM (SELECT tvid, ts_start, ts_end FROM {schema}.{existing_report} GROUP BY 1, 2, 3) AS exs
LEFT JOIN (SELECT tvid FROM {schema}.{new_report} GROUP BY 1) AS new
  ON exs.tvid = new.tvid
JOIN dev.detection.viewing_content_golden gt
  ON gt.tvid = exs.tvid
 AND gt.session_start = exs.ts_start
 AND gt.session_end = exs.ts_end
 AND gt.session_start >= '2025-07-28 07:00:00'
 AND gt.session_start < '2025-07-29 07:00:00'
WHERE new.tvid IS NULL
GROUP BY 1, 2, 3, 4, 5, 6
-- ORDER BY gt.tvid, gt.session_start
LIMIT 1000
-- GROUP BY 1
""").display()
...
spark.sql(f"""
SELECT exs.*
FROM (SELECT tvid, ts_start, ts_end, channel_callsign_tuner FROM {schema}.{existing_report} GROUP BY 1, 2, 3, 4) AS exs
LEFT JOIN (SELECT tvid FROM {schema}.{new_report} GROUP BY 1) AS new
  ON exs.tvid = new.tvid
WHERE new.tvid IS NULL
ORDER BY exs.tvid, exs.ts_start
LIMIT 1000
-- GROUP BY 1
""").display()
